...
import { listItemOrderBarangSchemaForm, type ListItemOrderBarangSchema, type listItemOrderBarangSchema } from '@/src/lib/validation/gudang/order-barang/list-item-order-barang-schema'
...
const tableName = 'po_detail'
const defaultUser = 'systemPOD';
type PaginationResponse = z.infer<ReturnType<typeof paginationResponseSchema<typeof listItemOrderBarangSchema>>>;

export class OrderBarangDetailService {
  ...
  static async getById(id: string): Promise<ListItemOrderBarangSchema> {
  ...
  static async insert(req: ListItemOrderBarangSchema): Promise<ListItemOrderBarangSchema> {
    const { ...form } = req ;
    ...
    static async update(orderBarang: ListItemOrderBarangSchema): Promise<ListItemOrderBarangSchema> {
    const form = listItemOrderBarangSchemaForm.parse(orderBarang);
    const { data: { user } } = await this.supabase.auth.getUser();
    const emailUser = user?.email ?? defaultUser;
